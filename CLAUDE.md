...
- **Version source**: `__version__` in `pyshamir/__init__.py` is read at build time by `pyproject.toml`'s `[tool.setuptools.dynamic] version = { attr = ... }`. Releases are cut by manually triggering the `Release` workflow (`.github/workflows/release.yml`) from the Actions tab and picking a bump type (`patch`/`minor`/`major`). The workflow uses `mathieudutour/github-tag-action` to compute the next semver tag, sed-updates `__version__` in its own working tree, builds, publishes to PyPI via OIDC, and creates the GitHub release. The workflow does **not** push the version bump back to `main`, so source-tree `__version__` will lag behind the latest tag — bump it via a normal PR when the drift becomes annoying.
