...
- **Version source**: `__version__` in `pyshamir/__init__.py` is read at build time by `pyproject.toml`'s `[tool.setuptools.dynamic] version = { attr = ... }`. **Don't bump it by hand.** Releases are managed by [release-please](https://github.com/googleapis/release-please) (see `release-please-config.json` and `.release-please-manifest.json`): merging Conventional-Commit-formatted commits (`feat:`, `fix:`, `perf:`, `revert:`, or `BREAKING CHANGE:`) to `main` causes release-please to open or update a "Release PR" that bumps `__version__`, regenerates `CHANGELOG.md`, and on merge creates the `vX.Y.Z` tag and GitHub release. Other Conventional types (`build:`, `ci:`, `chore:`, `docs:`, `style:`, `refactor:`, `test:`) don't trigger releases. To force a specific version, add `Release-As: X.Y.Z` to a commit body; for pre-releases use e.g. `Release-As: 1.1.0-rc1`.
