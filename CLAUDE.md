...
- **Version source**: `__version__` in `pyshamir/__init__.py` is read at build time by `pyproject.toml`'s `[tool.setuptools.dynamic] version = { attr = ... }`. Releases are cut by manually triggering the `Release` workflow (`.github/workflows/release.yml`) from the Actions tab. The workflow runs [`semantic-release`](https://github.com/semantic-release/semantic-release) configured via `.releaserc.json`, which inspects commits since the last tag and decides the next semver: `feat:` → minor, `fix:` → patch, `BREAKING CHANGE:` → major. Other Conventional Commit types (`chore:`, `ci:`, `docs:`, `build:`, `style:`, `refactor:`, `test:`, `perf:`) do NOT trigger a release. semantic-release pushes the new tag, sed-updates `__version__` in its working tree only, builds sdist + wheel via `@semantic-release/exec`, creates the GitHub release with the artifacts attached via `@semantic-release/github`, and a final workflow step publishes to PyPI via OIDC. The workflow does **not** push the version bump back to `main`, so source-tree `__version__` will lag behind the latest tag — bump it via a normal PR when the drift becomes annoying. If there are no releasable commits since the last tag, the workflow no-ops; force a release with an empty `fix:` commit if needed.
